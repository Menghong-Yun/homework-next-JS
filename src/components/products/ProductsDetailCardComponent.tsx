...
export default function ProductsDetailCardComponent({ product }: { product: Product }) {
  const [quantity, setQuantity] = useState(5);
  const originalPrice = (product.price * 1.5).toFixed(2);
  const discountPercent = Math.round(
    ((product.price * 1.5 - product.price) / (product.price * 1.5)) * 100
  );

  return (
    <section className="font-['Poppins'] flex items-center justify-center bg-black bg-opacity-20 min-h-[100vh] py-10">
      <div className="flex w-[1300px] p-10 bg-white border-gray-400 rounded-xl border-2">
        <div className="flex flex-col gap-14 items-center">
          ...
          <div className="flex flex-col gap-3">
            {[product.image, product.image, product.image, product.image].map((img, i) => (
              <img key={i} className="w-20 h-[90px] object-contain" src={img} alt={product.title} />
            ))}
          </div>
        ...
        <img className="w-[556px] ml-3 mr-6 object-contain" src={product.image} alt={product.title} />
        ...
        <div>
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <svg key={i} width={16} height={15} viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    ...
                    fill={i < Math.round(product.rating.rate) ? "#FF8A00" : "#E0E0E0"}
                  />
                </svg>
              ))}
            </span>
            <span className="ml-1 mr-3">{product.rating.count} Reviews . </span>
          ...
          <div className="h-[54px] mt-6 flex-col justify-start items-start gap-3 inline-flex">
            <div className="justify-start items-start gap-1.5 inline-flex">
              <span className="text-[#191919] text-sm font-medium leading-[21px]">Category:</span>
              <span className="text-[#7f7f7f] text-sm font-normal leading-[21px] capitalize">
                {product.category}
              </span>
            </div>
          </div>
